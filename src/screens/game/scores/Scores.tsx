...
const Scores: FC<TProps> = (props) => {
  const { open } = props;
  const scores = useScores();
  const { step } = useGameContext();

  return (
    <div className={`${styles.scores} ${open ? styles.active : ''}`}>
      {scores.map((score, key) => {
        ...
        return (
          <div key={score} className={`${styles.score} ${active} ${inactive} `}>
            <svg className={styles.border} width="240" height="40" viewBox="0 0 240 40" fill="none"
